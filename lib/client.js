...
function MqttClient (streamBuilder, options) {
  ...
  debug('[gabriel] MqttClient :: Gabriel\'s store processing reset fix')
...
MqttClient.prototype._onConnect = function (packet) {
  ...
  function startStreamProcess () {
    ...
    function clearStoreProcessing () {
      debug('[gabriel] clearStoreProcessing :: clearing')
      that._storeProcessing = false
      that._packetIdsDuringStoreProcessing = {}

      debug('[gabriel] clearStoreProcessing :: that._storeProcessing', that._storeProcessing)
    }
    ...
    function storeDeliver () {
      debug('[gabriel] storeDeliver :: was called!')
      // edge case, we wrapped this twice
      if (!outStore) {
        return
      }

      debug('[gabriel] storeDeliver :: setting _storeProcessing', true)
      ...
      if (!packet) {
        debug('[gabriel] storeDeliver :: if !packet')
        clearStoreProcessing()
      ...
      if (that._packetIdsDuringStoreProcessing[packet.messageId]) {
        debug('[gabriel] storeDeliver :: if that._packetIdsDuringStoreProcessing[packet.messageId]')
        storeDeliver()
        return
      }
    ...
    outStore.on('end', function () {
      ...
      debug('[gabriel] outStore :: allProcessed', allProcessed)
      debug('[gabriel] outStore :: that._packetIdsDuringStoreProcessing', that._packetIdsDuringStoreProcessing)

      if (allProcessed) {
        debug('[gabriel] outStore :: allProcessed! YES!')
        clearStoreProcessing()
        that.removeListener('close', remove)
        that.emit('connect', packet)
      } else {
        debug('[gabriel] outStore :: startStreamProcess AGAIN!')
        startStreamProcess()
      }
    })
    storeDeliver()
